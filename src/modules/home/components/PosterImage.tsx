...
const PosterImage = memo(
  ({
    ...
    const getColorsAsync = useCallback(async () => {
      try {
        if (!onFoundColor && !showPlayButton) {
          setIsLoading(false);
          return;
        }
        ...
        switch (result.platform) {
          case "ios":
            ...
            onFoundColor &&
              onFoundColor(index, [result.primary, result.secondary]);
            break;
          case "android":
            const vibrant = result.lightVibrant!;
            const dominant = result.dominant!;
            const average = result.average!;
            setColors([vibrant, average, dominant]);
    ...
    }, [uri, showPlayButton, setIsLoading, onFoundColor]);
    const handlePress = useCallback(() => {
      onPress && onPress(bookId, sharedAnimationPrefix || "");
    }, [onPress, bookId, sharedAnimationPrefix]);
    ...
    return (
      <Animated.View entering={enteringAnimation} style={containerStyles}>
        {isLoading && (
